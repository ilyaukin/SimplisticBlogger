...
class PostService(object):
    ...
    def add_post(self, blog_title, blog_author, blog_content, curr_user,
                 image_list, post_tags_list):
        try:
            ...
            db.session.commit()
            model_image_list = [
                images_model.Images(image_url=image_url, posts=posts) for
                image_url in image_list]
            ...
            db.session.commit()
            # Add the tags
            model_tags_list = [tags_model.Tags(tag=tag, posts=posts) for tag in
                               post_tags_list]
    ...
    def cmp_add(self, new_items, db_items, reference, db_type):
        for item in new_items:
            if item not in db_items:
                if db_type == "IMAGES":
                    db_obj = images_model.Images(image_url=item,
                                                 posts=reference)
                elif db_type == "TAG":
    ...
    def edit_post(self, post, db_tags, new_title, new_content, new_image_list,
                  new_tags_list):
        try:
            ...
            if ("uncategorized" not in new_tags_list) and (
                    "uncategorized" in db_tag_list):
                print("True")
                db_tag_list.remove("uncategorized")
                tag = tags_model.Tags.query.filter_by(tag="uncategorized",
                                                      posts=post).first()
    ...
    def get_all_posts(cls, order_by=False, is_admin=False):
        if is_admin and order_by:
            posts = posts_model.Posts.query.order_by(
                posts_model.Posts.posted_date.desc()).all()
        if is_admin:
