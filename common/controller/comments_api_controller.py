...
comments_bp = Blueprint("comments_api_bp", __name__)


def verify_recaptcha(recaptcha_response):
    url = "https://www.google.com/recaptcha/api/siteverify"
    payload = {'secret': os.environ.get("RECAPTCHA_SITE_SECRET"),
...
def add_comment():
    ...
    if len(post_data) > 0:
        ...
        if verify_recaptcha(g_recaptcha)["success"]:
            ...
            if comments_service_obj.add_comment(author_name, author_email,
                ...
                if is_admin:
                    ...
                    if c_status:
                        ...
                        return "Your comment is posted and notification sent " \
            ...
            # Internal error
            return "There has been an error while posting the comment, " \
                   "try after sometime..."
        # recaptcha failed
...
def approve_comment():
    ...
    if comment_ref_id and comment_status:
        ...
        resp = comment_serv_obj.edit_comment(comment_ref_id.split("-")[1],
                                             comment_status)
        print(resp)
        return jsonify(resp)
    else:
