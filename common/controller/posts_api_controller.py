...
def add():
    ...
    rc = p_service_obj.add_post(blog_title, blog_author, blog_content,
                                current_user, img_list, blog_tags_list)
    if rc:
        data_resp = {"redirect_uri": url_for("posts.dash_posts"),
                     "message": "Successfully inserted the post !!"}
        return jsonify(data_resp)
    return "There has been some error while inserting the post. Check " \
           "Logs and retry posting again", 500
...
def edit():
    ...
    # Call the post service to add the post
    post_data = posts_service.PostService.get_post_by_title(old_title,
                                                            is_admin=True)
    if len(post_data) > 0:
        p_obj = posts_service.PostService()
        post = post_data[0]
        tags = post_data[1]
        rc = p_obj.edit_post(post, tags, blog_title, blog_content, new_img_list,
                             blog_tags_list)
        if rc:
            return "Successfully edited the blog content"
        return "Could not edit the content of blog, check logs", 500
    else:
        return "No such post to edit", 500
...
def delete():
    ...
    # Call the post service to add the post
    post_data = posts_service.PostService.get_post_by_title(blog_title,
                                                            is_admin=True)
    if len(post_data) > 0:
        p_obj = posts_service.PostService()
        ...
        return "Could not delete the content of blog, check logs", 500

    else:
        return "No such post to delete, must be a hack !!!", 500
